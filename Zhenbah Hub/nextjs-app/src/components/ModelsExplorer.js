...
function ModelsExplorer({ models, providers }) {
  ...
  // Ensure models and providers are arrays
  const safeModels = Array.isArray(models) ? models : [];
  const safeProviders = Array.isArray(providers) ? providers : [];

  const categories = [
    { id: 'all', label: 'All Models', count: safeModels.length },
    { id: 'free', label: 'Free Models', count: safeModels.filter(m => m.pricing_tier === 'free').length },
    { id: 'pro', label: 'Pro Models', count: safeModels.filter(m => m.pricing_tier === 'pro').length },
    { id: 'text', label: 'Text Generation', count: safeModels.filter(m => m.category === 'text').length },
    { id: 'image', label: 'Image Generation', count: safeModels.filter(m => m.category === 'image').length },
    { id: 'audio', label: 'Audio Processing', count: safeModels.filter(m => m.category === 'audio').length },
    { id: 'video', label: 'Video Generation', count: safeModels.filter(m => m.category === 'video').length },
    { id: 'code', label: 'Code & Development', count: safeModels.filter(m => m.category === 'code').length },
    { id: 'multimodal', label: 'Multimodal', count: safeModels.filter(m => m.category === 'multimodal').length },
    { id: 'embeddings', label: 'Embeddings', count: safeModels.filter(m => m.category === 'embeddings').length }
  ];
  ...
  return (
    <div className="models-explorer">
      ...
      <div className="results-info">
        Showing {filteredModels.length} of {safeModels.length} models
      </div>
    </div>
  );
}
